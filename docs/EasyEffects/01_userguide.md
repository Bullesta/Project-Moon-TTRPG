...
- On Hit / On Hit Before Results: the attacker's used kit (weapon, applied tool, skill, ammo) and the attacker's statuses.
...
| `[On Hit]`                                                                                | An attack connects: one-sided, after a Clash Win, or a connecting Counter. Runs on the **attacker** weapon, applied tool, skill, ammo, and statuses. After `[Clash Win]` / `[Clash Lose]`. The attack has connected; damage has **not** necessarily been applied yet |
