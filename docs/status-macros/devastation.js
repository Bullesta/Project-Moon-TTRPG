/*
Project Moon TTRPG - Devastation status macro

Linked to a Devastation status item. This example consumes Devastation for
bonus HP damage when you trigger it manually. It also keeps the companion Ruin
status present if Devastation exists and Ruin is missing.
*/
...
if (context.event === 'onManualButton') {
  ...
  if (!targetActor) {
    ui.notifications.warn('Devastation needs a targetActorId in the payload to deal extra damage.');
    return;
  }

  const roll = await (new Roll(`${copies.length}d8`)).evaluate({ async: true });
  await dealDamage(targetActor, roll.total);
  await actor.deleteEmbeddedDocuments('Item', copies.map(item => item.id));
  console.log(`[PMTTRPG] Devastation spent by ${actor.name} on ${targetActor.name}.`);
}
